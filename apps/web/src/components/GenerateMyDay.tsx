...
import { useGenerateDay, useApplyAIAction, useUndoAIAction, useLatestAIAction, useEntitlements, canUseAI } from '@daypilot/lib';
...
export function GenerateMyDay({ onScheduleGenerated }: GenerateMyDayProps) {
  const [backlogTasks, setBacklogTasks] = useState<Array<{
    title: string;
    description?: string;
  ...
  const handleUndo = async () => {
    ...
    if (!confirm('Are you sure you want to undo the last AI-generated schedule?')) {
      return;
    }
  ...
  return (
    <Card>
      <div className="mb-4">
        <h2 className="text-2xl font-bold mb-2 text-[#2B3448]">Generate My Day</h2>
        <p className="text-gray-600 text-sm">
          Let AI create an optimized schedule for your day based on your tasks and existing events.
        </p>
      </div>

      {/* Backlog Tasks Input */}
      <div className="mb-4">
        <label className="block text-sm font-medium mb-2 text-[#2B3448]">Backlog Tasks (Optional)</label>
      ...
      {hasDraftAction && latestAction && (
        <div className="mb-4 p-4 bg-[#4FB3B3]/10 border border-[#4FB3B3] rounded-lg">
          <h3 className="font-semibold mb-3 text-[#2B3448]">Preview Schedule</h3>
          
          {/* Existing Events */}
          {todayEvents.length > 0 && (
            <div className="mb-4">
              <h4 className="text-sm font-medium mb-2 text-gray-600">Existing Events</h4>
          ...
          {/* Proposed Blocks */}
          <div className="mb-4">
            <h4 className="text-sm font-medium mb-2 text-gray-600">Proposed Schedule</h4>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {latestAction.output.blocks.map((block: AIBlock, index: number) => {
          ...
          {/* Conflicts */}
          {latestAction.output.conflicts && latestAction.output.conflicts.length > 0 && (
            <div className="mb-4 p-2 bg-yellow-50 border border-yellow-200 rounded">
              <h4 className="text-sm font-medium mb-1 text-yellow-800">Conflicts</h4>
