...
const plans = [
  ...
  { name: "Pro", price: "$12", period: "per month", description: "For professionals who need more", features: ["Everything in Free", "Unlimited booking links", "Calendar sync (Google, Outlook, Apple)", "Advanced AI scheduling", "Priority support", "Custom branding"], cta: "Start Free Trial", highlight: true },
  { name: "Team", price: "$29", period: "per month", description: "For teams and small businesses", features: ["Everything in Pro", "Team collaboration", "Shared calendars", "Organization management", "Multi-location support", "Admin controls"], cta: "Contact Sales", highlight: false },
  { name: "Enterprise", price: "Custom", period: "", description: "For large organizations", features: ["Everything in Team", "Unlimited users", "Custom integrations", "Dedicated support", "SLA guarantee", "Custom training"], cta: "Contact Sales", highlight: false },
];
  ...
  return (
    <nav className="section-padding py-4 md:py-6 flex justify-between items-center sticky top-0 z-50 glass-effect border-b border-white/20">
      ...
      <div className="hidden md:flex items-center gap-6">
        ...
        <Link href="/pricing" className="text-[#4FB3B3] font-medium text-sm md:text-base">Pricing</Link>
    ...
    <div className="min-h-screen">
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`glass-effect rounded-2xl p-6 md:p-8 flex flex-col ${
                plan.highlight ? "border-2 border-[#4FB3B3] shadow-xl scale-105 md:scale-110 relative" : "border border-white/20 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
              }`}
            >
              {plan.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="bg-gradient-to-r from-[#EFBF4D] to-[#4FB3B3] text-white text-xs font-semibold px-4 py-1.5 rounded-full shadow-lg">POPULAR</span>
                </div>
              )}
              <div className="flex-1">
                ...
                <div className="mb-2">
                  ...
                  {plan.period && <span className="text-[#4f4f4f] ml-2 text-base md:text-lg">/{plan.period}</span>}
                </div>
                ...
                <ul className="space-y-2.5 md:space-y-3 mb-8">
                  {plan.features.map((f, i) => (
                    <li key={i} className="flex items-start">
                      ...
                      <span className="text-sm md:text-base text-[#4f4f4f] leading-relaxed">{f}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <section className="container-width section-padding pb-16 md:pb-24 lg:pb-32 text-center">
        <div className="max-w-2xl mx-auto space-y-3 md:space-y-4 px-4">
          <p className="text-base md:text-lg text-[#4f4f4f]">All plans include a 14-day free trial. No credit card required.</p>
          <p className="text-sm text-[#4f4f4f]">
            Questions? <Link href="/" className="text-[#4FB3B3] hover:underline font-medium">Contact us</Link>
          </p>
        </div>
      </section>
    </div>
