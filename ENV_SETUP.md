...
- **404 in DevTools**: On GitHub Pages, when you open a deep link (e.g. `/app/integrations`) or refresh on that URL, the **document** request returns **HTTP 404** because GitHub serves `404.html` with a 404 status. The page still loads (the response body is the SPA), so the app works; the 404 in the console is expected and can be ignored. If you want **200** for those URLs (no 404 in DevTools), deploy to Netlify or Vercel instead—`netlify.toml` and `vercel.json` already have SPA rewrites that serve `index.html` with 200.
